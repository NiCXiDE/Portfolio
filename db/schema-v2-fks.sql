...
SET @ddl = IF(@fk_exists = 0,
  'ALTER TABLE piece_resources ADD CONSTRAINT fk_piece_resources_asset FOREIGN KEY (media_asset_id) REFERENCES media_assets (id) ON DELETE RESTRICT',
  'SELECT 1');
PREPARE stmt FROM @ddl; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @ddl = IF(@fk_exists = 0,
  'ALTER TABLE project_resources ADD CONSTRAINT fk_project_resources_asset FOREIGN KEY (media_asset_id) REFERENCES media_assets (id) ON DELETE RESTRICT',
  'SELECT 1');
PREPARE stmt FROM @ddl; EXECUTE stmt; DEALLOCATE PREPARE stmt;
