/**
 * LIVE ONLY (prepared — do not run until post content-apply approval):
 * add fk_testimonials_entity on portfolio.
 *
 * Requires:
 *   DATABASE_NAME=portfolio
 *   V2_FK_TESTIMONIALS_LIVE_APPROVED=1
 *
 * Does NOT accept V2_FK_TESTIMONIALS_REHEARSAL_APPROVED.
 * Does NOT run content apply.
 */
...
async function main() {
  ...
  if (effective !== LIVE_DB) {
    throw new Error(
      `[fk-live] ABORT: effective database must be "${LIVE_DB}" (got "${effective}")`,
    );
  }

  if (process.env.V2_FK_TESTIMONIALS_LIVE_APPROVED !== "1") {
    throw new Error(
      `[fk-live] ABORT: requires V2_FK_TESTIMONIALS_LIVE_APPROVED=1 ` +
        `(rehearsal approval variable is not accepted)`,
    );
  }

  if (process.env.V2_FK_TESTIMONIALS_REHEARSAL_APPROVED === "1") {
    throw new Error(
      `[fk-live] ABORT: V2_FK_TESTIMONIALS_REHEARSAL_APPROVED must not be set for live FK`,
    );
  }
  ...
  try {
    ...
    if (connectedDb !== LIVE_DB) {
      throw new Error(
        `[fk-live] ABORT: connected database="${connectedDb}" !== "${LIVE_DB}"`,
      );
    }
    ...
    if (fkExists !== 0) {
      throw new Error(
        `[fk-live] ABORT: fk_testimonials_entity FOREIGN KEY already exists`,
      );
    }
    ...
    if (!childCol[0]) {
      throw new Error(`[fk-live] ABORT: testimonials.entity_id missing`);
    }
    if (!parentCol[0]) {
      throw new Error(`[fk-live] ABORT: entities.id missing`);
    }

    console.log(`precheck_child=${JSON.stringify(childCol[0])}`);
    console.log(`precheck_parent=${JSON.stringify(parentCol[0])}`);

    if (String(childCol[0].COLUMN_TYPE) !== String(parentCol[0].COLUMN_TYPE)) {
      throw new Error(
        `[fk-live] ABORT: type mismatch child=${childCol[0].COLUMN_TYPE} parent=${parentCol[0].COLUMN_TYPE}`,
      );
    }

    const [entityCount] = (await conn.query(
      `SELECT COUNT(*) AS c FROM entities`,
    )) as [Array<{ c: number | string }>, unknown];
    if (Number(entityCount[0]?.c ?? 0) === 0) {
      throw new Error(
        `[fk-live] ABORT: entities empty — run content apply live before FK`,
      );
    }
    ...
    if (invalidRefs !== 0) {
      throw new Error(
        `[fk-live] ABORT: ${invalidRefs} testimonials.entity_id point to missing entities`,
      );
    }

    console.log("[fk-live] Executing ADD CONSTRAINT fk_testimonials_entity …");
    await conn.query(
    ...
    console.log("[fk-live] ALTER committed.");
    ...
    if (!fkAfter.length) {
      throw new Error(`[fk-live] ABORT: FK missing after ALTER`);
    }
    if (fkAfter[0].DELETE_RULE !== "SET NULL") {
      throw new Error(
        `[fk-live] ABORT: DELETE_RULE=${fkAfter[0].DELETE_RULE} expected SET NULL`,
      );
    }
  } finally {
    await conn.end();
